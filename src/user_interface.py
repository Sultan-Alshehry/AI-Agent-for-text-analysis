...
class AityApp(t.Tk):
    def __init__(self):
        ...
        self.example_analysis = {"sports.txt": """Keyword        Relevance
...
futsal                1%"""}
        ...
        for F in (Dashboard, FileSelection, Analysis, Compare):
            frame = F(
                self, self.total_docs, self.analysed_docs, self.ready_to_compare_docs
            )
            self.frames[F] = frame
...
class Dashboard(t.Frame):
    def __init__(self, master, total_docs, analysed_docs, ready_compare):
        ...
        t.Button(
            btn_frame, text="Documents", width=15, command=self.show_documents
        ).grid(row=0, column=0, padx=10)

        t.Button(btn_frame, text="Uploads", width=15, command=self.show_uploads).grid(
    ...
    def create_stat(self, parent, title, value):
        ...
        if isinstance(value, t.StringVar):
            ...
            t.Label(
                box, text=value, fg="white", bg="#1a1f5a", font=("Arial", 12, "bold")
            ).pack()
    ...
    def show_uploads(self):
        ...
        t.Button(upload_box, text="Choose file", command=self.upload_file).pack()
...
class FileSelection(t.Frame):
    def __init__(self, master, total_docs, analysed_docs, ready_compare):
        ...
        t.Label(self, text="Select Document", fg="white", bg="#0b0f3b").pack(pady=20)

        self.file_container = t.Frame(self, bg="#0b0f3b")
        self.file_container.pack()

        t.Button(
            self, text="⬅ Back", command=lambda: master.show_frame(Dashboard)
        ).pack(anchor="nw")
...
class Analysis(t.Frame):
    def __init__(self, master, total_docs, analysed_docs, ready_compare):
        ...
        self.label = t.Label(
            self, text="Analysis Results", fg="white", bg="#0b0f3b", font=("Arial", 14)
        )
        ...
        t.Button(
            self, text="⬅ Back", command=lambda: master.show_frame(FileSelection)
        ).pack(anchor="nw")
